...
def mfcc_process(path):
    ...
    mfcc_features = mfcc_features[:, :MIN_MFCC_TIME_LEN] # 固定長度，若做出檔案長度處理後可省略
...
def dataTest():
    ...
    for trueLabel in testDataSet.keys():
        ...
        for testData in singleTestDataSet:
            ...
            # Skip samples that is too short
            if(testData.shape[0] < MIN_MFCC_TIME_LEN):
                continue
